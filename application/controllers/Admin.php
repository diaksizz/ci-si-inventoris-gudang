...
class Admin extends CI_Controller{
  ...
  public function delete_barang($id_transaksi)
  {
    ...
    public function user_update_barang($id_transaksi)
    {
      $where = array('id_transaksi' => $id_transaksi);
      $data['data_barang_update'] = $this->M_admin->get_data('tb_barang_masuk',$where);
      $data['list_satuan'] = $this->M_admin->select('tb_satuan');
      ...
      $this->load->view('user/form_barangmasuk/form_update',$data);
      $this->load->view('user/templates/footer.php');
    }
  public function user_delete_barang($id_transaksi)
  {
    $where = array('id_transaksi' => $id_transaksi);
    $this->M_admin->delete('tb_barang_masuk',$where);
  ...
  public function user_proses_databarang_masuk_insert()
  {
    ...
    if($this->form_validation->run() == TRUE)
    {
      ...
      $data = array(
            ...
            'tanggal'      => $tanggal,
            'lokasi'       => $this->session->userdata('name'),
  ...
  public function user_barang_keluar()
  {
    ...
    $data['avatar'] = $this->M_admin->get_data_gambar('tb_upload_gambar_user',$this->session->userdata('name'));
    $this->load->view('user/templates/header.php');
    $this->load->view('user/perpindahan_barang/form_update',$data);
    $this->load->view('user/templates/footer.php');
  }
  ...
  public function user_proses_databarang_masuk_update()
  {
    // $this->form_validation->set_rules('lokasi','Lokasi','required');
    $this->form_validation->set_rules('kode_barang','Kode Barang','required');
    $this->form_validation->set_rules('nama_barang','Nama Barang','required');
    ...
    if($this->form_validation->run() == TRUE)
    {
      ...
      $this->load->view('user/templates/header.php');
      $this->load->view('user/form_barangmasuk/form_update');
      $this->load->view('user/templates/footer.php');
    }
  }
  ####################################
  ...
  public function proses_satuan_insert()
  {
    ...
    $this->form_validation->set_rules('nama_satuan','Kategori','trim|required|max_length[100]');
  ...
  public function user_proses_data_keluar()
  {
    $this->form_validation->set_rules('tanggal_keluar','Tanggal Keluar','trim|required');
    if($this->form_validation->run() === TRUE)
    {
      ...
      $this->load->view('user/perpindahan_barang/form_update/'.$id_transaksi);
    }
  ...
  public function report()
  {
      ...
      $list_data = $this->db->select('bm.kode_barang, bm.nama_barang, bm.satuan, bm.tanggal AS tanggal_masuk, 
            SUM(bk.jumlah) AS total_barang_keluar, 
            bm.jumlah AS jumlah_barang_masuk')
        ->from('tb_barang_masuk bm')
        ->join('tb_barang_keluar bk', 'bm.kode_barang = bk.kode_barang', 'left')
        ->group_by('bm.kode_barang, bm.nama_barang, bm.satuan, bm.tanggal, bm.jumlah')
        ->get()
        ->result();
  
      $safety_stock_per_produk = [];
  
      foreach ($list_data as $row) {
          // Inisialisasi variabel untuk perhitungan safety stock
          $max_penjualan_per_hari = 0;
          $total_penjualan_per_hari = [];
          $max_lead_time = 0;
          $total_lead_time = 0;
          $count_lead_time = 0;
  
          // Ambil data penjualan per hari dan lead time dari tabel barang keluar
          $barang_keluar = $this->db->select('tanggal_masuk, tanggal_keluar, jumlah')
          ...
          foreach ($barang_keluar as $bk) {
              $penjualan_hari = $bk->jumlah;
              $tanggal_masuk = DateTime::createFromFormat('d/m/Y', $bk->tanggal_masuk);
              $tanggal_keluar = DateTime::createFromFormat('d/m/Y', $bk->tanggal_keluar);
  
              // Validasi format tanggal
              if ($tanggal_masuk && $tanggal_keluar) {
                  // Hitung lead time dalam hari
                  $lead_time = $tanggal_keluar->diff($tanggal_masuk)->days;
  
                  // Update maksimum penjualan per hari
                  if ($penjualan_hari > $max_penjualan_per_hari) {
                      $max_penjualan_per_hari = $penjualan_hari;
                  }
  
                  // Update maksimum lead time
                  if ($lead_time > $max_lead_time) {
                      $max_lead_time = $lead_time;
                  }
  
                  // Simpan total penjualan per hari
                  $total_penjualan_per_hari[] = $penjualan_hari;
  
                  // Hitung total dan jumlah lead time untuk rata-rata
                  $total_lead_time += $lead_time;
                  $count_lead_time++;
              } else {
                  // Logika jika format tanggal salah
                  // Bisa ditambahkan tindakan lain jika diperlukan
                  continue;
              }
          }
  
          // Cek apakah jumlah total_penjualan_per_hari dan count_lead_time tidak nol sebelum menghitung rata-rata
          $rata_penjualan_per_hari = (count($total_penjualan_per_hari) > 0) ? array_sum($total_penjualan_per_hari) / count($total_penjualan_per_hari) : 0;
          $rata_lead_time = ($count_lead_time > 0) ? $total_lead_time / $count_lead_time : 0;
  
          // Hitung safety stock untuk produk ini
          $safety_stock = ($max_penjualan_per_hari * $max_lead_time) - ($rata_penjualan_per_hari * $rata_lead_time);
  
          // Simpan hasil safety stock untuk produk ini
          $safety_stock_per_produk[$row->kode_barang] = $safety_stock > 0 ? $safety_stock : 0;
      }
  
      $data['list_data'] = $list_data;
      $data['safety_stock_per_produk'] = $safety_stock_per_produk;
      $data['avatar'] = $this->M_admin->get_data_gambar('tb_upload_gambar_user', $this->session->userdata('name'));
